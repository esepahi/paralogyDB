...
<!--  --> 
    ...
    if(strlen($dquery) >= $min_length){ // if query length is more or equal minimum length then
        $resultNumber = 0; 
        $query = htmlspecialchars($dquery); 
		...
		if ($species == "any"){
            ...
            WHERE (`pantherid` = '$query' or `pfamid` = '$query') ") or die(mysql_error());
			}
		else {
			$raw_results = mysqli_query($con, "SELECT * FROM gene_domain
            WHERE species = '$species' and (`pantherid` = '$query' or `pfamid` = '$query') ") or die(mysql_error());			
		}
		$nnodes = array(); 
        if(mysqli_num_rows($raw_results) > 0){
		echo '<b>Table & network results for: '.$dquery.'</b><hr>';			
			?>
